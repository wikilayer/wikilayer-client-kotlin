...
class WikiChannelTest {
    ...
    @Test
    fun `a blocked live channel moves to the mirror and remembers it for reads`() =
        runBlocking {
            val mirror = MockWebServer()
            mirror.start()
            try {
                answer("", code = 451)
                mirror.enqueue(MockResponse.Builder().body("event: changed\ndata: {}\n").build())
                mirror.enqueue(MockResponse.Builder().body("""{"wikis":[],"has_more":false}""").build())
                val hosts =
                    WikiHostPool(
                        primary = server.url("/").toString(),
                ...
                assertNotNull(withTimeoutOrNull(DIAL_AGAIN_MILLIS) { channel.changes(1).first() })
                WikiApi(hosts, client, jacksonObjectMapper()).wikis()
